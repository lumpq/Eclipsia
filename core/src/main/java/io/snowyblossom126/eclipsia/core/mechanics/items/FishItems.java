package io.snowyblossom126.eclipsia.core.mechanics.items;

import io.snowyblossom126.eclipsia.EclipsiaPlugin;
import io.snowyblossom126.eclipsia.utilities.Mm;
import io.snowyblossom126.eclipsia.utilities.calc.FishGradeCalculator;
...
public class FishItems {
    ...
    public static ItemStack fish(Player p, String id, double length, double weight) {
        ...
        Component displayName = Mm.mm(section.getString("display.name"));
        List<Component> generalLore = Mm.mm(section.getStringList("display.general-lore"));
        List<Component> explainLore = Mm.mm(section.getStringList("display.explain-lore"));
        ...
        Component grade = Mm.mm(gradeStr);
        ...
        p.sendMessage(Mm.mm(String.format(
    ...
    public static ItemStack fishBook(String id) {
        ...
        loreComponents.add(Mm.mm("<italic:false><yellow>최소 길이 <white>: <dark_aqua>" + minLength + " " + lengthUnit));
        loreComponents.add(Mm.mm("<italic:false><yellow>최대 길이 <white>: <dark_aqua>" + maxLength + " " + lengthUnit));
        loreComponents.add(Component.empty());
        loreComponents.add(Mm.mm("<italic:false><yellow>최소 무게 <white>: <gold>" + minWeight + " " + weightUnit));
        loreComponents.add(Mm.mm("<italic:false><yellow>최대 무게 <white>: <gold>" + maxWeight + " " + weightUnit));
        loreComponents.add(Component.empty());

        var condition = section.getConfigurationSection("capture-condition");
        if (condition != null) {
            if (condition.getBoolean("always-can-capture", false)) {
                loreComponents.add(Mm.mm("<italic:false><blue>항상 잡힌다"));
            }

            List<Integer> canCaptureMonths = condition.getIntegerList("can-capture-month");
            if (!canCaptureMonths.isEmpty()) {
                loreComponents.add(Mm.mm("<italic:false><green>잡히는 달: <gray>" + formatMonthList(canCaptureMonths)));
            }
            ...
            if (cantCaptureMonthSection != null && cantCaptureMonthSection.getBoolean("enable", false)) {
                ...
                if (!cantCaptureMonths.isEmpty()) {
                    loreComponents.add(Mm.mm("<italic:false><red>잡히지 않는 달: <gray>" + formatMonthList(cantCaptureMonths)));
                }
            }
            ...
            if (largeObjectCaptureSection != null && largeObjectCaptureSection.getBoolean("enable", false)) {
                ...
                if (!largeMonths.isEmpty()) {
                    loreComponents.add(Mm.mm("<italic:false><aqua>큰 개체가 잡히는 달: <gray>" + formatMonthList(largeMonths)));
                }
            }
            ...
            if (smallObjectCaptureSection != null && smallObjectCaptureSection.getBoolean("enable", false)) {
                ...
                if (!smallMonths.isEmpty()) {
                    loreComponents.add(Mm.mm("<italic:false><light_purple>작은 개체가 잡히는 달: <gray>" + formatMonthList(smallMonths)));
                }
            }
        }

        loreComponents.add(Component.empty());
        loreComponents.addAll(Mm.mm(section.getStringList("display.explain-lore")));
        loreComponents.add(Component.empty());
        loreComponents.add(Mm.mm("<italic:false><white>" + id));
