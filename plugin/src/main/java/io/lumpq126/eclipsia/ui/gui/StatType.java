...
public enum StatType {
    ...
    AGILITY("민첩", Material.FEATHER, "<color:#66ffff>", (st, statPoint) -> {
        ...
        if (st >= 2000) {
            ...
            lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#117777>공격속도 + " + fmt2(atkSpeed) + "%"));  // 가장 진한 하늘
        }
    ...
    INTELLIGENCE("지능", Material.WRITABLE_BOOK, "<color:#9999ff>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>마법 공격력 증가 <color:#9999ff>+ 1.0%"));  // 조금 진한 파랑
        lore.add(Component.empty());

        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#6666cc>마법공격력 + " + fmt(st * 1.0) + "%"));  // 진한 파랑
