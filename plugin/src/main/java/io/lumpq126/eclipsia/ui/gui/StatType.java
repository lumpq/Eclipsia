...
public enum StatType {
    ...
    STRENGTH("근력", Material.IRON_SWORD, "<color:#ff6666>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>물리공격력증가 <color:#ffbbbb>+ 1.0"));    // 조금 진한 빨강
        if (st >= 500) lore.add(Mm.mm("<white>방어력 증가 <color:#ff8888>+ 1.0%"));  // 더 진한 빨강
        lore.add(Component.empty());

        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#ff4444>물리공격력 증가 + " + (st * 1.0)));  // 진한 빨강
        if (st >= 500) lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#cc0000>방어력 증가 + " + (st * 1.0) + "%"));  // 가장 진한 빨강
    ...
    CONSTITUTION("건강", Material.NETHERITE_CHESTPLATE, "<color:#66cc66>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#44aa44>체력 + " + (st * 1.0) + "%"));  // 진한 초록
        if (st >= 5000) lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#117711>물리공격력 증가 + " + (st * 1.0) + "%"));  // 가장 진한 초록
    ...
    AGILITY("민첩", Material.FEATHER, "<color:#66ffff>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>이동속도 증가 <color:#bbdddd>+ 0.2%"));  // 조금 진한 하늘
        if (st >= 2000) lore.add(Mm.mm("<white>공격속도 증가 <color:#88cccc>+ 0.04%"));  // 더 진한 하늘
        lore.add(Component.empty());

        double moveSpeed = Math.min(st, 2000) * 0.2;
        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#44aaaa>이동속도 + " + moveSpeed + "%"));  // 진한 하늘
        if (st >= 2000) {
            double atkSpeed = (st - 1999) * 0.04;
            lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#117777>공격속도 + " + atkSpeed + "%"));  // 가장 진한 하늘
        }
    ...
    DEXTERITY("재주", Material.BOW, "<color:#ffff66>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>치명타 확률 증가 <color:#ffffbb>+ 0.1%"));  // 조금 진한 노랑
        if (st >= 1000) lore.add(Mm.mm("<white>치명타 공격력 증가 <color:#dddd88>+ 1.0%"));  // 더 진한 노랑
        lore.add(Component.empty());

        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#aaaa44>치명타 확률 + " + (st * 0.1) + "%"));  // 진한 노랑
        if (st >= 1000) lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#777711>치명타 피해 + " + (st * 1.0) + "%"));  // 가장 진한 노랑
    ...
    WISDOM("지혜", Material.ENCHANTED_BOOK, "<color:#cc99ff>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>마나 재생 증가 <color:#c9a6ff>+ 1"));  // 조금 진한 보라
        lore.add(Component.empty());

        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#a86aff>마나 재생 + " + (st * 1.0)));  // 진한 보라
    ...
    INTELLIGENCE("지능", Material.WRITABLE_BOOK, "<color:#9999ff>", (st, statPoint) -> {
        ...
        lore.add(Mm.mm("<white>마법 공격력 증가 <color:#9999ff>+ 10%"));  // 조금 진한 파랑
        lore.add(Component.empty());

        lore.add(Mm.mm("<white>현재 적용중인 능력치 : </white><color:#6666cc>마법공격력 + " + (st * 10) + "%"));  // 진한 파랑
